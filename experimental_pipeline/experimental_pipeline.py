...
methods on synthetic causal data with known ground truth.
...
from explainability_models import (
    ...
    ShapleyFlowWrapper, CausalShapley, TrueShapley
)
...
def reconstruct_true_generator(y_generation_params: Dict):
    """
    Reconstruct the true Y generator function from saved parameters.
    
    Parameters:
    -----------
    y_generation_params : Dict
        Dictionary containing generation parameters
    
    Returns:
    --------
    callable : Function that computes Y from X
    """
    parent_indices = y_generation_params['parent_indices']
    coefficients = y_generation_params['coefficients']
    gen_type = y_generation_params['type']
    ...
    if gen_type == 'linear':
        ...
        func_types = y_generation_params['func_types']
        func_types = {int(k): v for k, v in func_types.items()}
...
def calculate_all_shapley_values(dataset_configs: List[Dict]):
    """
    Calculate Shapley values using all methods for all combinations.
    
    For each dataset:
      - TrueShapley (1 per dataset)
    ...
    for ds_idx, config in enumerate(dataset_configs, 1):
        ...
        # Load Y generation parameters and reconstruct true generator
        generator_params_path = SYNTHETIC_DIR / f"{filename}_y_params.json"
        with open(generator_params_path, 'r') as f:
            y_generation_params = json.load(f)
        
        true_generator = reconstruct_true_generator(y_generation_params)
        y_parent_indices = y_generation_params['parent_indices']
        
        # Calculate TrueShapley (ground truth)
        logging.info("  [Progress: TrueShapley] Calculating ground truth...")
        ...
        true_importance = true_shapley_explainer.get_feature_importance()
        
        # Save TrueShapley results
        true_dir = EXPLAINABILITY_DIR / filename / 'true'
        true_dir.mkdir(parents=True, exist_ok=True)
        
        np.save(true_dir / 'shapley_values.npy', true_shapley_values)
        true_importance.to_csv(true_dir / 'feature_importance.csv', index=False)
        logging.info(f"  ✓ TrueShapley completed")
        ...
        for model_name in ['lgbm', 'nn']:
            ...
            for discovery_method in ['pc', 'lingam']:
                ...
                # Calculate comparison metrics vs TrueShapley
                ...
                for method_name, shap_vals in methods_dict.items():
                    flat_true = true_shapley_values.flatten()
                    flat_method = shap_vals.flatten()
                    
                    correlation = np.corrcoef(flat_true, flat_method)[0, 1]
                    mae = np.abs(flat_true - flat_method).mean()
                    mse = ((flat_true - flat_method) ** 2).mean()
                ...
                metrics_path = comparison_dir / 'comparison_vs_true.json'
                with open(metrics_path, 'w') as f:
                    json.dump(comparison_metrics, f, indent=2)
                
                # Create visualization comparing methods
                methods_dict['True'] = true_shapley_values
                
                fig = plot_shapley_feature_comparison(
                    shapley_dict=methods_dict,
                    feature_names=X_train.columns.tolist(),
                    true_parents=y_parent_indices
                )

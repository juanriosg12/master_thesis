...
The two metrics are designed to be used together, not in isolation. As the results in Section~\ref{sec:results} will show, the features flagged by $\Delta M$ and those flagged by $D$ are largely non-overlapping populations, each revealing a structurally distinct type of graph-induced failure that the other metric cannot detect.
...
The five hidden confounders introduce spurious correlations that challenge the causal sufficiency assumption shared by both estimators, adding a layer of real-world noise to an otherwise controlled setting. PC exhibits high structural resilience, recovering 40 of 87 true $X \to X$ edges with precision 0.741 and F1 of 0.567. Thanks to the fact that conditional independence testing evaluates localized relationships, it can partially block extended confounder-induced association paths through careful conditioning on intermediate variables \citep{Spirtes2000}, allowing PC to maintain a structurally conservative graph relatively close to the true sparse structure.
...
Two cross-cutting patterns emerge from Tables~\ref{tab:baseline_synth}--\ref{tab:baseline_sachs} and are visible immediately in the scatter plots. First, sign disagreement ($D_{\text{base}}$) occurs more frequently than the magnitude numbers alone suggest, and when an attribution's sign flips, the magnitude divergence metric loses much of its interpretive meaning: comparing the absolute sizes of two attributions that point in opposite directions conflates a directional inversion with a scaling change, masking the more consequential failure. Sign inversions arise whenever a moderate per-feature change crosses zero, a threshold many near-zero attributions sit close to, while large magnitude changes are concentrated in a handful of dominant features. Second, the choice of discovered graph, PC or LiNGAM, matters little for the graph-versus-no-graph contrast: both discovered graphs perturb $\Delta M_{\text{base}}$ and $D_{\text{base}}$ by nearly identical amounts for a given method, so the dominant driver of deviation is the method's structural coupling to any causal graph, not the specific graph supplied.
...
Two patterns hold across all settings. First, the specific discovered graph supplied (PC or LiNGAM) has little effect on the baseline deviation: both perturb $\Delta M_{\text{base}}$ and $D_{\text{base}}$ by nearly identical amounts for a given method, so what matters is whether a causal graph is used at all, not which specific graph is supplied. Second, sign disagreement ($D$) is both more frequent and more consequential than magnitude divergence ($\Delta M$) alone can capture: when an attribution flips sign, comparing its absolute size to the reference loses interpretive meaning, because a magnitude number says nothing about whether the explanation reversed direction. Near-zero attributions need only a small shift to cross the sign boundary whereas large attributions absorb the same redistribution without changing direction, which is why $D$ fires broadly across features while $\Delta M$ concentrates on a few dominant nodes.
...
All research questions, experimental designs, analyses, tables, figures, and conclusions presented in this thesis are the original work of the author. Artificial intelligence tools were used exclusively as writing aids to check grammar, improve textual cohesion, and refine word choice. No AI tool was used to generate, interpret, or derive any of the scientific content, data, or ideas contained herein.

%% ============================================================
\section*{Declaration on the Use of Data}
\addcontentsline{toc}{section}{Declaration on the Use of Data}

The real-world dataset used in this thesis is the Sachs protein-signalling dataset, obtained from the following publicly available repository:

\begin{quote}
\textit{Example causal datasets: Sachs} [data repository]. Carnegie Mellon University, Department of Philosophy. \url{https://github.com/cmu-phil/example-causal-datasets/tree/main/real/sachs}, 2024.
\end{quote}

The dataset is used solely for academic research purposes in accordance with its terms of availability.

The experimental pipeline, code, and synthetic datasets produced for this thesis are publicly available at:

\begin{quote}
J.\ Rios, \textit{master\_thesis} [software repository]. GitHub. \url{https://github.com/juanriosg12/master_thesis}, 2026.
\end{quote}
...
%% ============================================================
\appendix
\stepcounter{section}% \thesection = A; subsections become A.1, A.2, …
\phantomsection
\addcontentsline{toc}{section}{Appendix: Shapley Method Subroutines}
\section*{Appendix: Shapley Method Subroutines}

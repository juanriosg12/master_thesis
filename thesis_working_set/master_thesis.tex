...
Both algorithms operate strictly on the $X$-only training partition, excluding the target for the dicovery step. While both the synthetic dataset and the real-world biological data likely do not perfectly fulfill all the theoretical requirements of these algorithms, this imperfection is deliberate. This is a realistic robustness test, reflecting the natural unmeasured confounding and structural uncertainty practitioners face in real-world applications.
...
The oracle analysis requires a known ground-truth graph and is only possible in controlled experiments. The question a practitioner actually faces without oracle access is how consistent the explanations are when two different discovery algorithms are tried on the same data. This final comparison answers that practical question: if a practitioner runs two discovery algorithms on the same data and feeds each graph into the same Shapley method, how much do the explanations change? The answer is, it depends heavily on the method, and the effect is larger on the sign of attributions than on their magnitude. Asymmetric Shapley consistently shows the lowest sensitivity, it keeps both $\Delta M_{\text{disc}}$ and $D_{\text{disc}}$ well below its counterparts on both datasets. The gap is most visible on the real data, Flow's magnitude divergence between the two graphs ($\Delta M_{\text{disc}} = 14.92\%$ of $\hat{\sigma}$) is nearly double Asymmetric's (8.46\%), meaning that choosing Shapley Flow instead of Asymmetric makes the explanation twice sensible as much in magnitude depending solely on which discovery algorithm was run. On the sign axis, choosing between Causal Shapley and Asymmetric Shapley for the Sachs dataset determines whether switching from one discovered graph to the other flips 31.80\% or 18.60\% of attribution signs, Causal disagrees with itself across graphs at almost double the rate of Asymmetric.
...
Adding the $D_{\text{disc}}$ for X10, the practitioner could spot a high sign disagreement between PC and LiNGAM,
making it validate the discovered causal structure and measuring the implication of this using the structure-aware Shapley method for this feature.
...
The three methods occupy distinct somehow expected sensitivity regimes, but now it is measurable. ASV is stable under graph errors: on the 50-feature synthetic data, replacing the True DAG with an imprecise discovered graph moves attributions by under 1.1\% of $\hat{\sigma}$ and flips fewer than 7\% of signs. On Sachs the absolute numbers are higher, oracle deviation rises to $\Delta M_{\text{oracle}} = 2.84\%$ of $\hat{\sigma}$ under LiNGAM (F1 = 0.326), but ASV still records the lowest oracle deviation of any method on that dataset, and remains the method least penalised by graph misspecification. CSV and Flow are more exposed because they embed the graph deeper into the computation, CSV inside the value function, Flow in the players themselves, so edge errors propagate directly into attributions.

Two patterns hold across all settings. First, the specific discovered graph supplied (PC or LiNGAM) has little effect on the baseline deviation: both perturb $\Delta M_{\text{base}}$ and $D_{\text{base}}$ by nearly identical amounts for a given method, so the dominant driver is the method's structural coupling to any graph, not which graph is used. Second, sign disagreement ($D$) is a more severe perturbation than magnitude divergence ($\Delta M$): adding a causal graph flips attribution directions more often than it inflates their size, because near-zero attributions need only a small shift to cross the sign boundary whereas large attributions absorb the same redistribution without changing direction.

Neither discovery algorithm consistently outperforms the other across both datasets, and in practice a discovered graph will always be imperfect. The decisive question is not which discovery algorithm is better, but how much the chosen Shapley method amplifies its errors. ASV absorbs those errors, especially in high-dimensional sparse graphs; CSV and Flow inherit them. This shifts the recommendation: before improving the graph, first check whether the Shapley method is sensitive enough to make that improvement worthwhile.

The metrics surface three distinct alarm patterns visible in the case studies. $\Delta M_{\text{base}}$ alone flags a correlated failure: X24's baseline deviation reaches 32.5\% of $\hat{\sigma}$ while its cross-discovery signal stays quiet, because both algorithms commit the same structural error and agree with each other despite both being wrong. $\Delta M_{\text{base}}$ and $\Delta M_{\text{disc}}$ firing together mark a contested neighbourhood: pka (17.3\% baseline), X47 (29.8\% cross-discovery for CSV), and erk (57.5\% cross-discovery for Flow) all show both alarms simultaneously, identifying nodes where the discovered causal structure is influential and disputed. The sign metrics surface a third class: X10 reaches $D_{\text{base}} = 96\%$ for Causal+PC and pkc leads the Sachs sign heatmaps, yet neither appears in the magnitude heatmaps — both carry near-zero baseline attributions, so a small structural shift is enough to flip their direction without leaving a large RMS footprint. A practitioner monitoring only magnitude metrics will miss this class entirely.
...
One practical use of the framework is to run two or more discovery algorithms on the same data and measure the cross-discovery stability ($\Delta M_{\text{disc}}$, $D_{\text{disc}}$) before committing to any structure-aware Shapley method. If PC and LiNGAM produce very different attributions for the same feature, that is a direct sign that the method's output depends more on which algorithm happened to be chosen than on the model's actual predictive behaviour. This does not require a ground-truth graph, the disagreement between two imperfect algorithms is itself the warning signal. This makes the pipeline a diagnostic tool: it points to exactly which nodes and which edges are responsible for the instability, so graph refinement effort can be focused where it matters. The sign metrics extend this audit: $\Delta M_{\text{disc}}$ flags nodes whose attribution magnitude is contested, $D_{\text{disc}}$ identifies a distinct population of near-zero features whose explanatory direction flips between algorithms — features that would be entirely invisible in a magnitude-only inspection.

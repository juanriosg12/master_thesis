...
On Sachs, this protection weakens substantially. With only 10 nodes, each mis-oriented edge eliminates a materially larger fraction of valid topological orderings, so ASV's averaging is no longer insensitive to discovery errors. The effect concentrates on erk: Flow under PC reaches a feature-level $\Delta M_{\text{oracle}}$ of 49.2\% of $\hat{\sigma}$, the worst single deviation in the experiment, but even Asymmetric under PC produces a feature-level $\Delta M_{\text{oracle}}$ of roughly 39\% on erk, a level comparable to what Causal and Flow produce on the synthetic track. This is not a failure of the method, it is a direct consequence of graph quality: PC deletes erk's true parent links entirely, and in a compact network that structural error forces even a permutation-restricted method to compute attributions over a very different set of admissible orderings. The general finding is that the ``Asymmetric rows are pale'' characterisation holds reliably on large, sparse graphs, but on small, dense networks discovery errors still propagate into ASV at the feature level when those errors are severe enough to restructure a node's local neighbourhood.
...
Two conclusions emerge from the oracle sign heatmaps. First, ASV's sparsity buffer is present on the synthetic track: global $D_{\text{oracle}}$ stays at roughly 6\% because the 50-node sparse graph leaves the vast majority of ordering constraints unchanged between the true and discovered graphs. However, this buffer does not suppress every feature equally, it fails precisely where a discovery error is structurally severe. X27 is isolated by both PC and LiNGAM (its edges in the True DAG are deleted entirely), so the ordering constraints that would normally anchor X27's relative position in the permutation space are absent under both discovered graphs, and ASV's oracle sign disagreement on X27 rises to roughly 42\%, comparable to Causal and Flow. The oracle sign heatmap therefore works as a structural audit tool for ASV: a feature with elevated $D_{\text{oracle}}$ under ASV is a direct indicator of a neighbourhood where the discovered graphs differ from the oracle. 
...
The oracle sign and magnitude heatmaps are therefore complementary diagnostics: high $\Delta M_{\text{oracle}}$ flags features whose importance is structurally overestimated or underestimated by the discovered graph, while high $D_{\text{oracle}}$ flags features whose explanability is inherently fragile, sitting close to zero under the oracle and thus susceptible to directional inversion from any non-trivial graph perturbation.
...
Causal Shapley carries the largest cross-discovery sign instability on both tracks, reaching up to 64\% disagreement on individual features, meaning that for 64 out of 100 evaluated instances the attribution sign flips depending solely on which discovery algorithm was used, which is a very high instability. This concentrates across methods on X10 and X13 for the synthetic dataset and on pkc and p38 for Sachs, features that are examined in detail in Section~\ref{sec:cases}.

...
Both algorithms operate strictly on the $X$-only training partition, excluding the target for the dicovery step. While both the synthetic dataset and the real-world biological data likely do not perfectly fulfill all the theoretical requirements of these algorithms, this imperfection is deliberate. This is a realistic robustness test, reflecting the natural unmeasured confounding and structural uncertainty practitioners face in deployment.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\linewidth}
    ...
    \caption{Synthetic dataset (top 10 features by mean $D_{\text{disc}}$). The Asymmetric row is crearly low; Causal shows the highest sign instability concentrated on X10 and X32, while Flow's sign instability is moderate compared to its magnitude sensitivity.}
    \label{fig:sss_synth}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.48\linewidth}

...
The metrics surface three archetypal alarm patterns, each defined by which signals fire,
and all detectable without a ground-truth graph. A \textbf{correlated failure} is
indicated when $\Delta M_{\text{base}}$ is large while $\Delta M_{\text{disc}}$ stays
quiet: both algorithms committed the same structural error and agree with each other
despite both being wrong. A \textbf{contested neighbourhood} fires when both
$\Delta M_{\text{base}}$ and $\Delta M_{\text{disc}}$ are elevated on the same feature:
the graph is doing substantial structural work \emph{and} the two algorithms disagree on
what that structure is. A \textbf{near-zero sign flip} appears when $D_{\text{base}}$ or
$D_{\text{disc}}$ is high with no matching magnitude footprint: a feature whose
Traditional Shapley value sits close to zero needs only a small graph-induced shift to
invert its sign, a class invisible to magnitude-only inspection. One case illustrates
each pattern.
...
alarm: $\Delta M_{\text{base}}$ fires while $\Delta M_{\text{disc}}$ stays quiet because
both algorithms converged on the same wrong structure, a failure the cross-discovery
comparison cannot detect on its own.
...
\subsubsection{Contested Neighbourhood and Structural Role Error}

Protein erk on Sachs is the clearest contested-neighbourhood case: both observable
alarm signals fire simultaneously, and the practitioner needs no ground-truth DAG to
spot the problem. In the consensus DAG, erk is a downstream node receiving inputs from
pka and mek. PC removes its true parent links entirely, leaving erk as an isolated
boundary source (Figure~\ref{fig:dag_erk}). LiNGAM, by contrast, recovers at least some
incoming connections, placing erk closer to its true downstream role. The two algorithms
therefore disagree fundamentally on erk's structural position.

Both observable signals flag this directly. First, $\Delta M_{\text{base}}$ is elevated
on erk across all three methods, indicating the injected graph is doing substantial
structural work on this protein's attribution. Second, $\Delta M_{\text{disc}}$ reaches
$57.5\%$ of $\hat{\sigma}$ for Flow and $35.8\%$ for ASV on erk --- the highest
cross-discovery divergence ASV produces for any feature on either dataset. A practitioner
running both algorithms would flag erk immediately from these signals alone, with no
oracle access required.  The SHAP scatter (Figure~\ref{fig:shap_erk}) confirms the
consequence: Flow+PC inflates erk by treating it as an isolated source; Flow+LiNGAM
produces a distinct distortion; neither matches the oracle.
...
\subsubsection{Near-Zero Sign Flip}

Feature X10 on the synthetic dataset illustrates the third alarm pattern. Its Traditional
Shapley attributions cluster near zero across the test set, so PC's structural changes
to X10's neighbourhood are enough to push most instances across the sign boundary,
producing $D_{\text{base}} = 96\%$ for Causal+PC --- the highest sign disagreement in
the synthetic experiment. Yet X10 is entirely absent from the magnitude heatmaps because
the absolute size of its attributions barely changes. The sign metrics
(Appendix~\ref{app:heatmaps_sign}) are the only signal that surfaces this class.
A practitioner monitoring only $\Delta M$ metrics will miss it entirely.
...
The two discovery algorithms swap rankings across datasets and neither reaches high
accuracy on either. Even so, $\Delta M_{\text{disc}}$ flags the same contested nodes
regardless of which algorithm performs better overall: on Sachs, ASV's
cross-discovery sensitivity is the lowest of the three methods, yet its feature-level
$\Delta M_{\text{disc}}$ still elevates on erk and pka, pointing a practitioner directly
to the proteins whose discovered causal connections need further inspection. The key
takeaway is that the practical decision is not about finding the best discovery algorithm:
it is more productive to first check how sensitive the chosen Shapley method is to graph
errors on the specific data at hand, and only invest in graph improvement where
sensitivity is high.

The evaluation metrics reveal structure that is invisible when looking at attributions
alone. $\Delta M_{\text{base}}$ flags nodes where the injected graph is doing substantial
work, a signal that the structure-aware method has moved meaningfully away from the
graph-free baseline on that feature. $\Delta M_{\text{disc}}$ flags nodes where the two
discovery algorithms disagree on local structure, meaning the attribution is sensitive to
which graph was chosen rather than to the model's predictive behaviour. When both signals
are elevated on the same node, the graph structure for that feature is simultaneously
influential and contested: the two-signal alarm that warrants structural validation before
trusting any structure-aware method for that node.

\subsection{Practical Recommendations}

The most important contribution is the evaluation framework itself. Before this work, a
practitioner adding a discovered graph to a Structure-aware Shapley computation had no systematic way
to assess what that decision would cost in attribution quality. The six metrics at both
dataset and feature level give a structured vocabulary for this assessment. The
...
\begin{itemize}[nosep]
  ...
  \item \textbf{CSV and Flow carry high risk and should only be used when the graph is
  specifically validated.} Acceptable conditions are: domain experts have reviewed and
  confirmed the contested causal connections identified by $\Delta M_{\text{disc}}$, or
  the cross-discovery stability metrics show near-zero instability on the features that
  matter most. Even then the risk is substantial: CSV+LiNGAM on confounded synthetic
  produces $D_{\text{oracle}} = 37.5\%$ and $D_{\text{disc}} = 37.9\%$, meaning sign
  disagreements are not minor rescalings but full attribution inversions, and in a real
  deployment the oracle DAG is unknown, so there is no safety net to detect how far
  those inversions deviate from the true causal structure. An explanation that flips
  sign on a third of features tells the opposite causal story to the one the model's
  underlying system supports, making it difficult to defend under any form of audit.
\end{itemize}

...
\twocolumn[{%
  ...
  \begin{minipage}{0.95\linewidth}
    ...
    emerge from the case studies, corresponding to three observable alarm patterns
...
quantifies how attribution quality degrades as a function of graph imperfection; without
one, a practitioner has no principled basis for deciding whether a discovered graph
improves or distorts explanations relative to the simpler graph-free baseline.
...
graph reflects the true structure. This drives a high $\Delta M_{\text{disc}}$, placing
...
substantial structural work on this node. The Flow SHAP scatter shows three distinct
...
outgoing edges while LiNGAM keeps pka near-root; both alarms direct the practitioner to
...
outgoing edges---recasting it as an apparent root source. Feature-level
...
but reversed to $X_{47} \to X_{21}$ by LiNGAM. Under PC, Causal Shapley and Flow
...
Protein erk on Sachs is the node where both observable alarm signals fire simultaneously
and with the highest intensity in the entire experiment. In the consensus DAG, erk is a
downstream node receiving inputs from pka and mek. PC removes its true parent links
entirely, leaving erk as an isolated boundary source (Figure~\ref{fig:dag_erk}). LiNGAM,
by contrast, recovers at least some incoming connections, placing erk closer to its true
downstream role. The two algorithms therefore disagree fundamentally on erk's structural
position.
...
the right direction relative to the oracle.
...
96\%$ for Causal+PC---the highest sign disagreement in the synthetic experiment. Yet X10
is entirely absent from the magnitude heatmaps because the absolute size of its
attributions barely changes. The sign metrics (Appendix~\ref{app:heatmaps_sign}) are
the only signal that surfaces this class.

The same mechanism appears on Sachs through protein pkc, which leads the sign heatmaps
...
directly to proteins whose causal connections need validation. The evaluation metrics
...
Running all four metrics together makes the framework diagnostic: each pattern reveals
a failure mode the others cannot detect.

\subsection{Main Contribution and Practical Recommendations}

The most important contribution is the evaluation framework itself: the first systematic
...
graph densities, and larger sample sizes remain to be tested. Future work should extend
to nonlinear Shapley variants, explore ensemble approaches averaging attributions across
